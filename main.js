...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initialize() {
        ...
        audioLoader.load('assets/sounds/hover.wav', function(buffer) {
                 ...
                 if (shouldBeVisible) {
                    ...
                    if (item.lineElement) {
                        ...
                        if (item.lineElement.style.display === 'none' || item.isAnimatingOut) {
                            ...
                            gsap.to(item.lineElement.style, { strokeDashoffset: 0, duration: 0.4, ease: "power2.out" });
                            gsap.to(label, { opacity: 1, duration: 0.4, delay: 0.2 });
                        }
                    }

                 } else {
                     // Hide line and label with animation
                     if (item.lineElement && item.lineElement.style.display !== 'none') {
                         if (!gsap.isTweening(item.lineElement.style) && item.lineElement.style.strokeDashoffset === '0') {
                         ...
                         } else if (item.lineElement.style.strokeDashoffset === '0') {
                             // Force hide if stuck
                             // item.lineElement.style.display = 'none';
                         }
                     }
